...
char **splitString(char *inputStr, char *delimiterStr)
{
	...
	for (i = 0; inputStr[i] != '\0'; i++)
		if (!isDelim(inputStr[i], delimiterStr) &&
				(isDelim(inputStr[i + 1], delimiterStr) || !inputStr[i + 1]))
			numWords++;
	if (numWords == 0)
		return (NULL);
	...
	{
		while (isDelim(inputStr[i], delimiterStr))
			i++;
		k = 0;
		while (!isDelim(inputStr[i + k], delimiterStr) && inputStr[i + k])
			k++;
